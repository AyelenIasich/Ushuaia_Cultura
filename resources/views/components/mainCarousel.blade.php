                ...
                @foreach ($home as $hom)
                    @foreach ($hom->home_images as $img)
                        <div class="carousel-item active">
                            <img src="{{ asset('carousel') . '/' . $img->image }}" class="d-block w-100" alt="...">
                        </div>
                    @endforeach
                                        ...
                                        @if (Auth::check())
                                            <div class="col col-md-4  col-xxl-2  text-start ms-0 ps-0">
                                                <a href="{{ url('/home/' . $hom->id . '/edit') }}"
                                                    class="btn btn-success"><i class="fa-solid fa-pen"></i> Editar</a>
                                            </div>
                                        @endif
                                    </div>
                                </div>
                ...
                <div class="col ms-3 me-3">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="content ">
                                @foreach ($home as $hom)
                                    <h1 class=" card-title text-center"><span>Ushuaia,</span>{{ $hom->titulo }}</h1>
                                    <div class=" card-text descripcion text-center">
                                        <p class="mt-3 ">{{ $hom->descripcion }}</p>
                                    </div>
                                @endforeach

                                <div class="row botones">
                                    <div class="col-12 text-center mt-2"><a class="btn-success btn" href="{{ url('/#artistas' ) }}">Quiero ser parte
                                        </a>
